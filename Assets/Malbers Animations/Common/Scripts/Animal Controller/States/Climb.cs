    ...
    public class Climb : State
    {
        ...
        public override void OnStateMove(float deltatime)
        {
            if (InCoreAnimation)
            {
                ...
                animal.DirectionalVelocity = animal.Up * animal.VerticalSmooth + animal.Right * animal.HorizontalSmooth; //IMPORTANT OF ADDITIONAL SPEED
        ...
        public override void TryExitState(float DeltaTime)
        {
            if (InCoreAnimation)
            {
                ...
                {
                    if (Physics.Raycast(MainPivot, Gravity, out _, scalefactor * GroundDistance, animal.GroundLayer, QueryTriggerInteraction.Ignore)) //Means that the Animal is going down and touching the ground
                    {
                ...
                //if (animal.MovementAxisRaw.z > 0) //Means the animal is going UP
                {
                    bool LedgeHit = Physics.RaycastNonAlloc(LedgePivotUP, forward, EdgeHit, scalefactor * RayLedgeLength, animal.GroundLayer, QueryTriggerInteraction.Ignore) > 0;
                    ...
                    if (!LedgeHit)
                    {
                        ...
                        LedgeHit = Physics.RaycastNonAlloc(SecondRayPivot, Gravity, EdgeHit, scalefactor * RayLedgeLength, animal.GroundLayer, QueryTriggerInteraction.Ignore) > 0;
